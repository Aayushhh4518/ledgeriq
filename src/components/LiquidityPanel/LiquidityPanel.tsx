      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800/50">
          ...
          <p className={`text-2xl font-bold tracking-tight ${
            (liquidity.currentRatio?.value ?? 0) >= 1.5 ? "text-emerald-400" : (liquidity.currentRatio?.value ?? 0) >= 1 ? "text-amber-400" : "text-rose-400"
          }`}>
            {liquidity.currentRatio?.value !== undefined ? liquidity.currentRatio.value.toFixed(2) : "N/A"}x
          </p>
          <p className="text-[10px] text-zinc-500 mt-2">Target: &gt; 1.5x</p>
        </div>

        <div className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800/50">
          <p className="text-xs font-semibold tracking-wider text-zinc-500 uppercase mb-2">Debt to Equity</p>
          <p className={`text-2xl font-bold tracking-tight ${
            (liquidity.debtToEquity?.value ?? Infinity) <= 1.5 ? "text-emerald-400" : (liquidity.debtToEquity?.value ?? Infinity) <= 2.5 ? "text-amber-400" : "text-rose-400"
          }`}>
            {liquidity.debtToEquity?.value !== undefined ? liquidity.debtToEquity.value.toFixed(2) : "N/A"}x
          </p>
          <p className="text-[10px] text-zinc-500 mt-2">Target: &lt; 1.5x</p>
        </div>
      </div>
